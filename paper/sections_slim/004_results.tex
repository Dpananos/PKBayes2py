...
\begin{equation}
Q_{1}^{o p t}\left(H_{1}, A_{1}\right)= E \left[Y_1\left(c_{1}(A_1),  \cdots, c_{K}(A_1)\right)+\max _{a_{2}} Q_{2}^{o p t}\left(H_{2}, a_{2}\right) \Bigg\vert H_{1}, A_{1}\right]
\end{equation}

We seek to maximize the stage 1 optimal Q function to learn the optimal DTR for dosing subjects under the constraint we can measure them at most once and are limited to the aforementioned pre-dose clinical variables.  The interpretation of stage 1 optimal Q function is as follows:\textit{ Given the pre-dose clinical variables of the subject and a proposed initial dose and measurement time, the stage 1 optimal Q function gives the expected proportion of time the subject’s blood serum concentration is between 100 ng/ml and 300 ng/ml assuming that we provide the subject with the best dose possible at the start of the $ 5^{th} $ day.}  The decision rules which choose $ A_1 $ and $ A_2 $ to maximize these functions constitutes the estimated optimal DTR.
...
Modes of personalization which use less information have larger regret.  The One Size Fits All approach (which uses no information about the subject) performs worst with a median regret of 0.145.  The distribution of regrets for this mode is right skewed with some exceeding 0.95, meaning the subject could have been in range for nearly the entire time if the correct PK parameters were known, but the mode selected a dose which failed to put the subject in range. 
